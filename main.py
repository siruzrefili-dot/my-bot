...
class Config:
    ...
    SHORT_RSI_MAX = 60.0
    BOS_BUFFER_ATR = 0.05
    ALLOW_RECLAIM_BOS = True
    BOS_LOOKBACK = 60
    RETEST_MAX_BARS = 6
    RETEST_ATR_DISTANCE = 0.60
    CONFIRMATION_MAX_BARS_AFTER_RETEST = 3
    TARGET_LOOKBACK_15 = 80
    TARGET_LOOKBACK_1H = 80
    TARGET_LOOKBACK_4H = 80
    ...
    FALLBACK_COINS = ["BTCUSDT","ETHUSDT","SOLUSDT","BNBUSDT","XRPUSDT",
                      "ADAUSDT","AVAXUSDT","DOGEUSDT","LINKUSDT","SUIUSDT",
                      "ARBUSDT","OPUSDT"]
    TRADFI = {"XAU","XAG","CL","USOIL","UKOIL","SPX","NDX","DJI","US30",
              "NAS100","US500","GER40","UK100","JP225","HK50","AAPL","AMZN",
              "AMD","COIN","GOOG","GOOGL","META","MSFT","MSTR","MU","NFLX",
              "NVDA","ORCL","PLTR","QCOM","TSLA","TSM","HOOD","SMCI","RKLB",
              "OPENAI","NBIS","HPE","ANTHROPIC","CRWV","ADBE","NOKIA","LITE",
              "ASTS","AEHR","ALAB","COHR","CIEN","POET","BNC","AXTI","FLNC",
              "CRCL","BE","ONDS","CBRS","PURR","STXX","QNTX","SKHYNIX","SNDK","SOXL"}
...
class BybitClient:
    ...
    def get(self, path, params=None, key=None, cache=None, retries=3):
        ...
        for attempt in range(retries):
            try:
                ...
                z = r.json()
                if z.get("retCode") != 0:
                    raise RuntimeError(z.get("retMsg", "API error"))
                if key:
                    c.set(key, z)
                return z
        ...
        log.warning("API %s: %s", path, last_err)
        return None

    def klines(self, symbol, interval, limit=300):
        z = self.get("/v5/market/kline",
                     {"category": "linear", "symbol": symbol,
                      "interval": interval, "limit": limit},
                     f"k:{symbol}:{interval}:{limit}")
    ...
    def ticker(self, symbol, fresh=False):
        c = None if fresh else self.tcache
        k = None if fresh else f"t:{symbol}"
        z = self.get("/v5/market/tickers",
                     {"category": "linear", "symbol": symbol}, k, c)
        a = z.get("result", {}).get("list", []) if z else []
        if not a:
            return {}
        a = a[0]
        return {"symbol": a.get("symbol", ""),
                "last_price": safe_float(a.get("lastPrice")),
    ...
    def all_tickers(self):
        z = self.get("/v5/market/tickers", {"category": "linear"},
                     "ALL_LINEAR_TICKERS", self.tcache)
        return z.get("result", {}).get("list", []) if z else []

    def oi(self, symbol, limit=10):
        z = self.get("/v5/market/open-interest",
                     {"category": "linear", "symbol": symbol,
    ...
    def book(self, symbol):
        z = self.get("/v5/market/orderbook",
    ...
    def instrument(self, symbol):
        ...
        if z is not None:
            return z
        x = self.get("/v5/market/instruments-info",
                     {"category": "linear", "symbol": symbol},
                     f"i:{symbol}")
        ...
        a = a[0]
        p = a.get("priceFilter", {})
        q = a.get("lotSizeFilter", {})
        z = {"tick": safe_float(p.get("tickSize")),
             "step": safe_float(q.get("qtyStep")),
             "min": safe_float(q.get("minOrderQty")),
             "status": a.get("status", ""),
             "contractType": a.get("contractType", ""),
             "quoteCoin": a.get("quoteCoin", ""),
             "settleCoin": a.get("settleCoin", ""),
             "baseCoin": a.get("baseCoin", ""),
             "symbol": a.get("symbol", "")}
        self.icache.set(symbol, z)
        return z
...
def validate_config():
    if not 0 <= Config.MIN_SCORE <= 100 or Config.MIN_RR <= 0 or Config.MAX_RR < Config.MIN_RR:
        raise ValueError("score/rr config")
...
class Structure:
    ...
    def swings(df, l=5, r=5, cache_key=None):
        if cache_key:
            with Structure._cache_lock:
                c = Structure._swing_cache.get(cache_key)
                if c is not None:
                    return c
        h = df.high.to_numpy()
        lo = df.low.to_numpy()
        ...
        for i in range(l, n - r):
            if h[i] >= max(h[i-l:i]) and h[i] > max(h[i+1:i+r+1]):
                sh.append((i, float(h[i])))
            if lo[i] <= min(lo[i-l:i]) and lo[i] < min(lo[i+1:i+r+1]):
                sl.append((i, float(lo[i])))
        if cache_key:
            with Structure._cache_lock:
                Structure._swing_cache[cache_key] = (sh, sl)
                if len(Structure._swing_cache) > 200:
                    for k in list(Structure._swing_cache.keys())[:50]:
                        Structure._swing_cache.pop(k, None)
        return sh, sl
...
class Regime:
    ...
    def analyze(df):
        if len(df) < 220:
            return {"direction": "neutral", "quality": 0, "structure": "neutral"}
        x = df.iloc[-1]
...
class Strategy:
    ...
    def pullback(df, d):
        if len(df) < 8:
            return False
        x = df.iloc[-7:-1]
        ema = safe_float(df.ema_fast.iloc[-1])
        atr = safe_float(df.atr.iloc[-1])
        if atr <= 0:
            return False
        if d == "long":
            return bool((x.low <= ema + atr * 1.5).any())
    ...
    def rsi_allowed(df, d):
        ...
        if not math.isfinite(r):
            return False
        if d == "long":
            return Config.LONG_RSI_MIN <= r <= Config.LONG_RSI_MAX
    ...
    def rsi_score(df, d):
        ...
        if d == "long":
            if 48 <= r <= 58:
                return 100
            if Config.LONG_RSI_MIN <= r < 48 or 58 < r <= 62:
                return 80
            return 65
        if 42 <= r <= 52:
            return 100
        if 38 <= r < 42 or 52 < r <= 55:
            return 80
        return 65
    ...
    def strong(df, i, d):
        ...
        if body < atr * 0.25:
            return False
        if d == "long":
            return bool(x.close > x.open and x.close >= p.close
                        and x.close >= x.high - atr * 0.30)
        return bool(x.close < x.open and x.close <= p.close
                    and x.close <= x.low + atr * 0.30)

    @staticmethod
    def bos(df, d):
        sh, sl = Structure.swings(df)
        n = len(df)
        sw = sh if d == "long" else sl
        cut = max(0, n - Config.BOS_LOOKBACK - 15)
        cands = []
        for si, level in sw:
            if si < cut or si >= n - 3:
                continue
            for i in range(si + 1, n):
                ...
                if not math.isfinite(atr) or atr <= 0:
                    continue
                x = df.iloc[i]
                buf = atr * Config.BOS_BUFFER_ATR
                if d == "long":
                    close_break = x.close >= level + buf
                    reclaim = (Config.ALLOW_RECLAIM_BOS and
                               Strategy.strong(df, i, d) and
                               x.close >= level and x.high >= level)
                else:
                    close_break = x.close <= level - buf
                    reclaim = (Config.ALLOW_RECLAIM_BOS and
                               Strategy.strong(df, i, d) and
                               x.close <= level and x.low <= level)
                if close_break or reclaim:
                    cands.append({"level": level, "bar": i, "swing": si,
                                  "type": "CLOSE_BOS" if close_break else "RECLAIM_BOS"})
                    break
        if not cands:
            return None
        best = max(cands, key=lambda z: z["bar"])
        age = n - 1 - best["bar"]
        max_age = (Config.RETEST_MAX_BARS +
                   Config.CONFIRMATION_MAX_BARS_AFTER_RETEST + 2)
    ...
    def sequence(df, bos, d):
        ...
        for i in range(start, min(len(df), start + Config.RETEST_MAX_BARS + 1)):
            ...
            if not (touch and hold):
                continue
            for j in range(i + 1, min(len(df), i + 1 +
                                       Config.CONFIRMATION_MAX_BARS_AFTER_RETEST + 1)):
                if Strategy.strong(df, j, d):
                    if d == "long" and df.close.iloc[j] > lv:
                        return {"bos": bos["bar"], "retest": i,
                                "confirm": j, "level": lv}
...
class Filters:
    ...
    @staticmethod
    def volume(df):
        if not valid(df, Config.VOLUME_LOOKBACK + 2):
            return False
        v = safe_float(df.volume_ratio.iloc[-1], math.nan)
    ...
    @staticmethod
    def oi(df):
        if not valid(df, 2) or "oi_value" not in df:
            return False
    ...
    def allowed(symbol, active):
        ...
        for s in active:
            ...
            try:
                ...
                if not valid(a, 40) or not valid(b, 40):
                    continue
                c = a.close.pct_change().tail(40).corr(b.close.pct_change().tail(40))
                if math.isfinite(safe_float(c, math.nan)) and c >= Config.CORRELATION_THRESHOLD:
                    n += 1
            except Exception:
                continue
...
class Risk:
    ...
    def levels(d15, d1, d4, d):
        e = safe_float(d15.close.iloc[-1])
        atr = safe_float(d15.atr.iloc[-1])
        if e <= 0 or atr <= 0:
            return None
        raw = Risk.structural_stop(d15, e, d)
        min_risk = atr * Config.MIN_SL_ATR
        max_risk = atr * Config.MAX_SL_ATR
        if d == "long":
            sl = min(raw, e - min_risk)
        else:
            sl = max(raw, e + min_risk)
        risk = abs(e - sl)
        ...
        if d == "long":
            lo = e + risk * Config.MIN_RR
            hi = e + risk * Config.MAX_RR
        else:
            lo = e - risk * Config.MIN_RR
            hi = e - risk * Config.MAX_RR
        for level in cs:
            if d == "long":
                tp = level - atr * Config.TARGET_BUFFER_ATR
                if not (lo <= tp <= hi):
                    continue
            else:
                tp = level + atr * Config.TARGET_BUFFER_ATR
...
class Scoring:
    ...
    def calculate(d4, d15, d, seq):
        rsi_score = Strategy.rsi_score(d15, d)
        vol = safe_float(d15.volume_ratio.iloc[-1])
        vol_score = min(100, vol * 70)
        atr_pct = safe_float(d15.atr_pct.iloc[-1])
        atr_score = 100 if atr_pct >= Config.MIN_ATR_PCT else 0
        regime_q = Regime.analyze(d4)["quality"]
        zone = Strategy.zone_score(d15, d)
        setup_score = 90
        score = (0.25 * regime_q + 0.15 * setup_score +
                 0.25 * (100 if seq else 0) + 0.15 * rsi_score +
                 0.10 * zone + 0.05 * vol_score + 0.05 * atr_score)
        return round(score, 1), rsi_score
...
class Analyzer:
    ...
    def run(self):
        s = self.symbol
        d4 = BYBIT.klines(s, "240", 300)
        d1 = BYBIT.klines(s, "60", 300)
        d15 = BYBIT.klines(s, "15", 300)
        ok = valid(d4, 220) and valid(d1, 220) and valid(d15, 80)
        STORE.add_stage("DATA", ok)
        if not ok:
            self.reject = "DATA"
            return None
        d4 = Indicators.add(d4)
        d1 = Indicators.add(d1)
        ...
        checks = [(d != "neutral", "4H_TREND"),
                  ...
                  (Strategy.rsi_allowed(d15, d), "RSI"),
        ...
        if not ok:
            self.reject = "ENTRY_CHASE"
            return None
        ok = Filters.volume(d15)
        STORE.add_stage("VOLUME", ok)
        if not ok:
            self.reject = "VOLUME"
            return None
        t = BYBIT.ticker(s)
        ok = bool(t)
        STORE.add_stage("TICKER", ok)
        if not ok:
            self.reject = "TICKER"
            return None
        ok = Filters.funding(t)
        STORE.add_stage("FUNDING", ok)
        if not ok:
            self.reject = "FUNDING"
            return None
        ok = Filters.oi(BYBIT.oi(s))
        STORE.add_stage("OI", ok)
        if not ok:
            self.reject = "OI"
            return None
        ok = Filters.spread(BYBIT.book(s))
        STORE.add_stage("SPREAD", ok)
        if not ok:
            self.reject = "SPREAD"
            return None
        ok = BTCFilter.allowed(d)
        STORE.add_stage("BTC_FILTER", ok)
        if not ok:
            self.reject = "BTC_FILTER"
            return None
        lv = Risk.levels(d15, d1, d4, d)
        ok = bool(lv)
        STORE.add_stage("TARGET_RISK", ok)
        if not ok:
            self.reject = "TARGET_RISK"
            return None
        score, rsi_s = Scoring.calculate(d4, d15, d, seq)
        ok = score >= Config.MIN_SCORE
        STORE.add_stage("SCORE", ok)
        if not ok:
            self.reject = "SCORE"
            return None
        return {"symbol": s, "direction": d, "score": score,
                "rsi": safe_float(d15.rsi.iloc[-1]), "rsi_score": rsi_s,
                "entry": lv["entry"], "sl": lv["sl"], "tp": lv["tp"],
                "rr": lv["rr"], "risk": lv["risk"],
                "atr_pct": safe_float(d15.atr_pct.iloc[-1]),
                "volume_ratio": safe_float(d15.volume_ratio.iloc[-1]),
                "funding": safe_float(t.get("funding"), 0),
                "oi": safe_float(t.get("oi"), 0),
...
class Store:
    ...
    def add_bos_debug(self, name):
        with self.lock:
            q = self.stages.setdefault("BOS_DEBUG", {})
            q[name] = q.get(name, 0) + 1
    def bos_report(self):
        with self.lock:
            q = dict(self.stages.get("BOS_DEBUG", {}))
        return " | ".join(f"{k}: {v}" for k, v in q.items()) or "Yoxdur"
    def stage_report(self, n):
        order = ["DATA","4H_TREND","1H_SETUP","1H_PULLBACK","RSI","ATR",
                 "BOS","RETEST_CONFIRM","CONFIRM_AGE","ENTRY_CHASE",
                 "VOLUME","TICKER","FUNDING","OI","SPREAD",
                 "BTC_FILTER","TARGET_RISK","SCORE"]
        with self.lock:
            q = dict(self.stages)
        lines = []
        for x in order:
            p = q.get(x, {}).get("pass", 0)
            f = q.get(x, {}).get("fail", 0)
            lines.append(f"{x}: {p}/{p+f} keçdi | {f} keçmədi")
        return "\n".join(lines)
    def add_rejection(self, s, r):
        with self.lock:
            self.rejections[s] = r or "UNKNOWN"
    def rejection_stats(self):
        with self.lock:
            q = {}
            for v in self.rejections.values():
                q[v] = q.get(v, 0) + 1
            return q
    def active_symbols(self):
        with self.lock:
            return [x["symbol"] for x in self.active]
    def can_add(self):
        self.reset_day()
        with self.lock:
    ...
    def add(self, x):
        with self.lock:
            if len(self.active) >= Config.MAX_ACTIVE_SIGNALS:
                return False
            if self.daily_count >= Config.MAX_DAILY_SIGNALS:
                return False
            if any(a["symbol"] == x["symbol"] for a in self.active):
                return False
    ...
    def load(self):
        try:
            with open(Config.SIGNAL_FILE, encoding="utf8") as f:
                x = json.load(f)
            self.active = x.get("active", [])
            self.closed = x.get("closed", [])
            self.day = x.get("day", self.day)
            self.daily_count = x.get("daily_count", 0)
            self.reset_day()
        except Exception:
            pass
...
class Performance:
    ...
    def stats():
        c = STORE.closed
        w = sum(x.get("result") == "TP" for x in c)
        l = sum(x.get("result") == "SL" for x in c)
        t = w + l
        pnl = sum(safe_float(x.get("pnl")) for x in c)
        return {"closed": t, "wins": w, "losses": l,
                "winrate": round(w / t * 100, 2) if t else 0,
                "pnl": round(pnl, 2), "today_signals": STORE.daily_count}
...
class Telegram:
    ...
    def status():
        if not STORE.active:
            return "📭 Aktiv siqnal yoxdur."
        lines = ["📌 AKTİV SIQNALLAR\n"]
        for x in STORE.active:
            lines.append(f"{x['symbol']} {x['direction'].upper()}\n"
                         f"Entry: {x['entry']:.8g}\nSL: {x['sl']:.8g}\n"
                         f"TP: {x['tp']:.8g}\nRR: 1:{x['rr']:.2f}")
        return "\n".join(lines)
    @staticmethod
...
class PositionManager:
    def check(self, x):
        ...
        age = (time.time() - x["created_ts"]) / 3600
        result = None
        if age >= Config.MAX_HOLD_HOURS:
            result = "TIME"
        elif d == "long" and p <= sl:
            result = "SL"
        elif d == "long" and p >= tp:
            result = "TP"
        elif d == "short" and p >= sl:
            result = "SL"
        elif d == "short" and p <= tp:
            result = "TP"
        if result:
            if result == "TP":
                rr = x["rr"]
                ...
                rr = (p - e) / (e - sl) if d == "long" else (e - p) / (sl - e)
            x["result"] = result
            x["result_r"] = rr
            x["exit_price"] = p
            x["pnl"] = rr * (Config.ACCOUNT_BALANCE * Config.RISK_PERCENT / 100)
            STORE.remove(x["symbol"], x)
            Telegram.send(f"🔔 CLOSED\n{x['symbol']} {d.upper()}\n"
                          f"Result: {result}\nExit: {p:.8g}\n"
                          f"R: {rr:.2f}\nPnL: {x['pnl']:.2f} USDT")
    def run(self):
        while not STOP_EVENT.is_set():
            try:
                for x in list(STORE.active):
                    self.check(x)
            except Exception as e:
    ...
    def symbols(self):
        try:
            ...
            for x in BYBIT.all_tickers():
                s = x.get("symbol", "")
                turn = safe_float(x.get("turnover24h"))
            ...
            for s, _ in c[:Config.CANDIDATE_LIMIT]:
                ...
                if (i and i.get("status") == "Trading" and
                    i.get("contractType") == "LinearPerpetual" and
                    i.get("quoteCoin") == "USDT" and
                    i.get("settleCoin") == "USDT"):
                    out.append(s)
                if len(out) >= Config.SCAN_TOP_N:
                    break
            return out or Config.FALLBACK_COINS
        except Exception as e:
            log.warning("symbols: %s", e)
        ...
        try:
            ...
            for x in found:
                ...
                if not Correlation.allowed(x["symbol"], STORE.active_symbols()):
                    STORE.add_rejection(x["symbol"], "CORRELATION")
                    continue
                if STORE.add(x):
                    Telegram.signal(x)
                    sent += 1
...
class TelegramPoller:
    ...
    def run(self):
        ...
        while not STOP_EVENT.is_set():
            try:
                data = requests.get(url, params={"timeout": 20, "offset": self.offset},
                                    timeout=25).json()
                for u in data.get("result", []):
                    self.offset = u["update_id"] + 1
                    m = u.get("message", {})
                    if str(m.get("chat", {}).get("id", "")) != str(Config.CHAT_ID):
                        continue
                    text = m.get("text", "")
                    if text.strip().lower() == "/scan":
                        if SCANNER.running:
                            Telegram.send("⏳ Scan artıq işləyir.")
                        else:
...
@app.get("/")
def home():
    return jsonify({"bot": BOT_VERSION, "status": "running",
                    "active": len(STORE.active), "today": STORE.daily_count})
@app.get("/status")
def web_status():
    return jsonify({"version": BOT_VERSION, "active": STORE.active,
...
@app.get("/rejections")
def web_rejections():
    return jsonify(STORE.rejection_stats())
...
def startup():
    ...
    Telegram.send(f"🤖 SWING AI {BOT_VERSION}\n4H → 1H → 15M\n"
                  ...
                  f"ATR ≥ {Config.MIN_ATR_PCT:.2f}% | SCORE ≥ {Config.MIN_SCORE}\n"
                  f"RR {Config.MIN_RR:.1f}-{Config.MAX_RR:.1f} | "
                  f"DAILY {Config.MAX_DAILY_SIGNALS}")
