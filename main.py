...
BOT_VERSION="10.8.1 STABLE FINAL"
...
def pct(a,b):return abs(a-b)/b*100 if b else math.nan
def valid(df,n):return isinstance(df,pd.DataFrame) and len(df)>=n and all(c in df for c in ["open","high","low","close","volume"])
def savej(path,obj):
 try:
...
class Bybit:
 ...
 def spread(self,symbol):
  ...
  return pct(a,b) if math.isfinite(a) and math.isfinite(b) and b>0 else math.nan
...
def top_symbols():
 ...
 for x in BYBIT.instruments():
  ...
  t=BYBIT.ticker(s);v=safe(t.get("turnover24h"))
...
class Regime:
 ...
 def analyze(df):
  ...
  long_ok=ema_long and st!="bearish";short_ok=ema_short and st!="bullish"
...
class Strategy:
 ...
 def bos(df,d):
  ...
  for si,level in sw:
   ...
   for i in range(si+1,n):
    ...
    reclaim=False;crossed=(x.high>=level) if d=="long" else (x.low<=level)
 ...
 def retest_confirm(df,bos,d):
  ...
  n=len(df);atr=safe(df.atr.iloc[-1]);start=bos["bar"]+1;end=min(n-1,start+Config.RETEST_MAX_BARS);ret=None
...
class Filters:
 ...
 @staticmethod
 def ticker(symbol):
  x=BYBIT.ticker(symbol);p=safe(x.get("lastPrice"));b=safe(x.get("bid1Price"));a=safe(x.get("ask1Price"))
  return p>0 and b>0 and a>0
...
class Store:
 def __init__(self):
  ...
  self.active=loadj(Config.SIGNAL_FILE,[])
 ...
 def add_stage(self,name,ok):
  with self.lock:
   z=self.stages.setdefault(name,{"pass":0,"fail":0});z["pass" if ok else "fail"]+=1
 ...
 def stage_report(self,n=0):
  ...
  out=[]
  for x in order:
   z=q.get(x,{"pass":0,"fail":0});p=z["pass"];f=z["fail"];out.append(f"{x}: {p}/{p+f} keçdi | {f} keçmədi")
  return "\n".join(out)
...
def analyze_symbol(symbol):
 try:
  ...
  return {
   ...
   "risk":lv["risk"],"rr":lv["rr"],"score":round(score,1),"rsi":safe(x.rsi),
   "atr_pct":safe(x.atr_pct),"volume_ratio":safe(x.volume_ratio),"bos_type":bos["type"],
   "bos_level":bos["level"],"confirm_age":age,"trend_structure":reg["structure"],
   "trend_quality":round(reg["quality"],1)
  },None
 except Exception as e:
  log.error("analyze %s: %s",symbol,e);return None,"ERROR"
...
class Telegram:
 ...
 def send(text):
  ...
  try:
   u=f"https://api.telegram.org/bot{Config.BOT_TOKEN}/sendMessage"
   r=requests.post(u,data={"chat_id":Config.CHAT_ID,"text":text},timeout=10)
   if not r.ok:log.error("telegram http %s %s",r.status_code,r.text[:200])
   return r.ok
 ...
 def signal(x):
  ...
  return Telegram.send(
   ...
   f"BOS: {x['bos_type']}\n4H Structure: {x['trend_structure']}"
  )
...
class PositionManager:
 ...
 def check(self):
  ...
  for x in items:
   t=BYBIT.ticker(x["symbol"],fresh=True);p=safe(t.get("lastPrice"))
   if p<=0:continue
   hit=None
   if x["direction"]=="LONG":
    if p<=x["sl"]:hit="SL"
