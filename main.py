...
BOT_VERSION="10.9 BALANCED BOS FINAL"
class Config:
...
class Strategy:
 ...
 def setup(df,d):
  ...
  x=df.iloc[-1];return bool(x.close>x.ema_fast>x.ema_slow) if d=="long" else bool(x.close<x.ema_fast<x.ema_slow)
 @staticmethod
 def pullback(df,d):
 ...
 def rsi_score(df,d):
  ...
  if d=="long":return 100 if 48<=r<=58 else 80 if Config.LONG_RSI_MIN<=r<48 or 58<r<=62 else 65
  return 100 if 42<=r<=52 else 80 if 38<=r<42 or 52<r<=55 else 65
 @staticmethod
 def strong(df,i,d):
  if i<1 or i>=len(df):return False
  x=df.iloc[i];p=df.iloc[i-1];atr=safe_float(x.atr)
  if atr<=0 or abs(x.close-x.open)<atr*.30:return False
  return bool(x.close>x.open and x.close>=p.close and x.close>=x.high-atr*.25) if d=="long" else bool(x.close<x.open and x.close<=p.close and x.close<=x.low+atr*.25)
 @staticmethod
 def bos(df,d):
  n=len(df);sh,sl=Structure.swings(df);sw=sh if d=="long" else sl;cut=max(0,n-Config.BOS_LOOKBACK-15);cands=[]
  for si,level in sw:
   ...
   for i in range(si+1,n):
    ...
    rc=Strategy.strong(df,i,d) and ((x.close>=level) if d=="long" else (x.close<=level)) and ((x.high>=level) if d=="long" else (x.low<=level))
    if cb or rc:cands.append({"level":level,"bar":i,"swing":si,"type":"CLOSE_BOS" if cb else "RECLAIM_BOS"});break
  recent=18;start=max(0,n-recent-1)
  if n>recent+2:
   level=float(df.high.iloc[start:-1].max()) if d=="long" else float(df.low.iloc[start:-1].min())
   for i in range(start+1,n):
    atr=safe_float(df.atr.iloc[i],math.nan)
    if not math.isfinite(atr) or atr<=0:continue
    x=df.iloc[i];buf=atr*Config.BOS_BUFFER_ATR;cb=x.close>=level+buf if d=="long" else x.close<=level-buf;rc=Strategy.strong(df,i,d) and ((x.close>=level) if d=="long" else (x.close<=level))
    if cb or (Config.ALLOW_RECLAIM_BOS and rc):cands.append({"level":level,"bar":i,"swing":start,"type":"RECENT_BOS" if cb else "RECENT_RECLAIM"});break
  if not cands:return None
  best=max(cands,key=lambda z:z["bar"]);age=n-1-best["bar"]
  return best if age<=Config.RETEST_MAX_BARS+Config.CONFIRMATION_MAX_BARS_AFTER_RETEST+1 else None
 @staticmethod
