...
BOT_VERSION="9.6 BALANCED PRO"
class Config:
 BOT_TOKEN=os.getenv("BOT_TOKEN","");CHAT_ID=os.getenv("CHAT_ID","1121794078");BASE_URL="https://api.bybit.com";CATEGORY="linear"
 SCAN_TOP_N=40;MAX_SIGNALS_TO_SEND=3;CHECK_INTERVAL=300;MONITOR_INTERVAL=5;PARALLEL_WORKERS=6
 TREND_TF="240";SETUP_TF="60";ENTRY_TF="15";MONITOR_TF="1";EMA_FAST=50;EMA_SLOW=200;RSI_PERIOD=14;ATR_PERIOD=14;SWING_LOOKBACK=5
 MIN_SCORE=62;MIN_RR=2.;MAX_RR=3.;ACCOUNT_BALANCE=1000.;RISK_PERCENT=1.;MAX_ACTIVE_SIGNALS=3;MAX_DAILY_SIGNALS=5
 COMMISSION_PERCENT=.04;SLIPPAGE_PERCENT=.02;MIN_ATR_PCT=.15;MAX_ATR_PCT=8.;VOLUME_LOOKBACK=20;MIN_VOLUME_RATIO=1.05;MIN_EMA_DISTANCE_PCT=.10
 FUNDING_FILTER_ENABLED=True;MAX_ABS_FUNDING=.002;OI_FILTER_ENABLED=True;OI_LOOKBACK=5;MIN_OI_CHANGE_PCT=-8.
 ...
 DATA_DIR="swing_bot_data";FLASK_PORT=int(os.getenv("PORT","10000"));REQUEST_TIMEOUT=15;CACHE_TTL=10;TICKER_CACHE_TTL=30;INSTRUMENT_CACHE_TTL=3600
...
class BybitClient:
 ...
 def get(self,path,params=None,key=None,cache=None):
  if key:
   x=(cache or self.cache).get(key)
   if x is not None:return x
  try:
   r=self.session.get(self.base+path,params=params or {},timeout=Config.REQUEST_TIMEOUT);r.raise_for_status();x=r.json()
   if x.get("retCode")!=0:raise RuntimeError(x.get("retMsg","API error"))
   if key:(cache or self.cache).set(key,x)
   return x
  except Exception as e:log.warning("API %s: %s",path,e);return None
 def klines(self,symbol,interval,limit=300):
  x=self.get("/v5/market/kline",{"category":Config.CATEGORY,"symbol":symbol,"interval":interval,"limit":limit},f"k:{symbol}:{interval}:{limit}")
  rows=x.get("result",{}).get("list",[]) if x else []
  if not rows:return pd.DataFrame()
  rows=list(reversed(rows));c=["timestamp","open","high","low","close","volume","turnover"];df=pd.DataFrame(rows,columns=c)
  for z in c:df[z]=pd.to_numeric(df[z],errors="coerce")
  df=df.dropna(subset=["open","high","low","close","volume"])
  return df.iloc[:-1].reset_index(drop=True) if len(df)>1 else df.reset_index(drop=True)
 def ticker(self,symbol):
  x=self.get("/v5/market/tickers",{"category":Config.CATEGORY,"symbol":symbol},f"t:{symbol}",self.tcache);a=x.get("result",{}).get("list",[]) if x else []
  if not a:return {}
  a=a[0];return {"symbol":a.get("symbol",""),"last_price":safe_float(a.get("lastPrice")),"change":safe_float(a.get("price24hPcnt"))*100,"funding":safe_float(a.get("fundingRate")),"oi":safe_float(a.get("openInterest")),"turnover24h":safe_float(a.get("turnover24h"))}
 def all_tickers(self):
  x=self.get("/v5/market/tickers",{"category":Config.CATEGORY},"ALL_LINEAR_TICKERS",self.tcache);return x.get("result",{}).get("list",[]) if x else []
 def oi(self,symbol,limit=10):
  x=self.get("/v5/market/open-interest",{"category":Config.CATEGORY,"symbol":symbol,"intervalTime":"1h","limit":limit},f"oi:{symbol}:{limit}");rows=x.get("result",{}).get("list",[]) if x else []
  if not rows:return pd.DataFrame()
 ...
 def book(self,symbol):
  x=self.get("/v5/market/orderbook",{"category":Config.CATEGORY,"symbol":symbol,"limit":25},f"b:{symbol}");return x.get("result",{}) if x else {}
 def instrument(self,symbol):
  ...
  if not a:return {}
  a=a[0];p=a.get("priceFilter",{});q=a.get("lotSizeFilter",{});z={"tick":safe_float(p.get("tickSize")),"step":safe_float(q.get("qtyStep")),"min":safe_float(q.get("minOrderQty")),"max":safe_float(q.get("maxOrderQty"))};self.icache.set(symbol,z);return z
BYBIT=BybitClient()
...
class Regime:
 ...
 def analyze(df):
  ...
  x=df.iloc[-1];st=Structure.trend(df);dist=abs(safe_float(x.ema_distance_pct));atr=safe_float(x.atr_pct);q=min(dist,1)*40+min(atr/3,1)*20+(40 if st!="neutral" else 0)
  lo=x.close>x.ema_slow and x.ema_fast>x.ema_slow and st=="bullish" and dist>=Config.MIN_EMA_DISTANCE_PCT
  sh=x.close<x.ema_slow and x.ema_fast<x.ema_slow and st=="bearish" and dist>=Config.MIN_EMA_DISTANCE_PCT
  return {"direction":"long" if lo else "short" if sh else "neutral","quality":min(100,q),"structure":st}
class Strategy:
 @staticmethod
 def setup(df,d):
  if len(df)<100:return False
  x=df.iloc[-1]
  return bool(x.ema_fast>x.ema_slow and x.close>x.ema_fast) if d=="long" else bool(x.ema_fast<x.ema_slow and x.close<x.ema_fast)
 @staticmethod
 def pullback(df,d):
 ...
 def rsi_ok(df,d):
  ...
  a=safe_float(df.rsi.iloc[-2]);b=safe_float(df.rsi.iloc[-1]);return b>=47 and b>=a if d=="long" else b<=53 and b<=a
 @staticmethod
 def bos(df,d):
  sh,sl=Structure.swings(df)
  if d=="long" and sh:
   ...
   for i in range(sh[-1][0]+1,len(df)):
    if df.close.iloc[i]>lv:return {"level":lv,"bar":i}
  if d=="short" and sl:
   lv=sl[-1][1]
 ...
 def sequence(df,bos,d):
  ...
  for i in range(start,min(len(df),start+Config.RETEST_MAX_BARS)):
   ...
   if atr<=0:continue
   touch=x.low<=lv+atr*.6 if d=="long" else x.high>=lv-atr*.6
 ...
 @staticmethod
 def target(e,sl,d):
  r=abs(e-sl);return e+r*Config.MIN_RR if d=="long" else e-r*Config.MIN_RR
 @staticmethod
 def zone(df,d):
  hi=df.high.iloc[-50:].max();lo=df.low.iloc[-50:].min();mid=(hi+lo)/2;p=df.close.iloc[-1];return p<=mid if d=="long" else p>=mid
class RejectReport:
...
class Filters:
 @staticmethod
 def funding(v):return not Config.FUNDING_FILTER_ENABLED or abs(safe_float(v))<=Config.MAX_ABS_FUNDING
 @staticmethod
 def oi_change(df):
  if df is None or len(df)<2:return 0.
  a=safe_float(df.openInterest.iloc[0]);b=safe_float(df.openInterest.iloc[-1]);return (b-a)/a*100 if a>0 else 0.
 @staticmethod
 def oi(v):return not Config.OI_FILTER_ENABLED or v>=Config.MIN_OI_CHANGE_PCT
 @staticmethod
 def btc(v,d,s):
  if not Config.BTC_FILTER_ENABLED or s=="BTCUSDT" or abs(v)<.5:return True
  return v>0 if d=="long" else v<0
 @staticmethod
 def spread(book):
  try:
...
class Risk:
 @staticmethod
 def leverage(atr,e):
  r=atr/e if e>0 else .05;return min(2 if r>.05 else 3 if r>.03 else 4 if r>.02 else 5,Config.DEFAULT_LEVERAGE)
 @staticmethod
 def size(e,sl,lev):
  d=abs(e-sl)
  if e<=0 or d<=0:return {}
  risk=Config.ACCOUNT_BALANCE*Config.RISK_PERCENT/100;qty=min(risk/d,(Config.ACCOUNT_BALANCE*max(1,lev))/e);return {"risk":risk,"qty":qty}
 @staticmethod
 def costs(n):return n*(Config.COMMISSION_PERCENT+Config.SLIPPAGE_PERCENT)/100
class Precision:
 @staticmethod
 def price(v,t):
  return v if t<=0 else round(math.floor(v/t+1e-12)*t,12)
 @staticmethod
...
class Correlation:
 @staticmethod
 def allowed(client,tracker,symbol,direction):
  ...
  for s in active:
   if s["symbol"]==symbol or s["direction"]!=direction:continue
...
class SignalScanner:
 ...
 def analyze(self,s):
  try:
   ...
   if seq["confirm"]!=len(d15)-1:return self.reject(s,"OLD_CONFIRM")
   if not Strategy.zone(d15,d):return self.reject(s,"ZONE")
   ...
   e=safe_float(d15.close.iloc[-1]);atr=safe_float(d15.atr.iloc[-1]);lev=Risk.leverage(atr,e);sl=Strategy.stop(d15,e,d)
   ...
   if tick>0:e=Precision.price(e,tick);sl=Precision.price(sl,tick)
   ...
   maxdist=dist*Config.MAX_RR
   if abs(tp-e)>maxdist:tp=e+maxdist if d=="long" else e-maxdist
   if tick>0:tp=Precision.price(tp,tick)
   ...
   btc=safe_float(self.client.ticker("BTCUSDT").get("change")) if s!="BTCUSDT" else 0
   if not Filters.btc(btc,d,s):return self.reject(s,"BTC")
   spread=Filters.spread(self.client.book(s))
   if spread>.25:return self.reject(s,"SPREAD")
   if not Correlation.allowed(self.client,self.tracker,s,d):return self.reject(s,"CORRELATION")
   ...
   score=min(100,quality*.25+vs*.15+ois*.10+rrs*.20+momentum*.10+ss*.10+10)
   ...
   return {"signal":{"id":f"{s}_{d}_{utc_timestamp()}","symbol":s,"direction":d,"entry":e,"sl":sl,"tp":tp,"rr":round(rr,2),"score":round(score,2),"leverage":lev,"risk_amount":rd["risk"],"position_size":qty,"notional":qty*e,"estimated_cost":Risk.costs(qty*e),"atr":atr,"volume_ratio":vol,"oi_change_pct":oi,"funding_rate":fund,"spread_pct":spread,"sequence":"BOS -> RETEST -> CONFIRMATION","bos_level":seq["level"],"created_at":utc_iso(),"status":"NEW"},"symbol":s,"reason":None}
 ...
 def scan(self,symbols):
  ...
  with ThreadPoolExecutor(max_workers=Config.PARALLEL_WORKERS) as ex:
   ...
   for f in as_completed(fs):
    try:
     z=f.result()
     if z["signal"]:out.append(z["signal"])
...
class PerformanceTracker:
 ...
 def save(self):
  try:
   with self.lock:
    a=[x for x in self.signals if x.get("status")=="ACTIVE"];c=[x for x in self.signals if x.get("status")!="ACTIVE"][-Config.MAX_CLOSED_SIGNALS_KEPT:];fd,tmp=tempfile.mkstemp(dir=Config.DATA_DIR);os.close(fd)
 ...
 def can_add(self,s):
  today=utc_now().date().isoformat();daily=sum(str(x.get("created_at","")).startswith(today) for x in self.signals)
 ...
 def add(self,s):
  with self.lock:
   ...
   x=dict(s);x.update({"status":"ACTIVE","activated_at":utc_iso(),"original_sl":s["sl"],"original_risk":abs(s["entry"]-s["sl"]),"current_r":0.,"partial_taken":False,"breakeven":False,"trailing":False});self.signals.append(x);self.recalc();self.save();return True
 def close(self,sid,result,r,price,reason,cost=0):
  with self.lock:
   x=self.get(sid)
   if not x or x.get("status")!="ACTIVE":return False
   risk=safe_float(x.get("risk_amount"),1);net=r-cost/risk;result="WIN" if net>.05 else "LOSS" if net<-.05 else "BE";net=0 if result=="BE" else net
   x.update({"status":"CLOSED","result":result,"result_r":round(net,4),"gross_r":round(r,4),"close_price":price,"close_reason":reason,"cost":cost,"closed_at":utc_iso()});self.recalc();self.save();return True
 def recalc(self):
  c=[x for x in self.signals if x.get("status")=="CLOSED"];w=[x for x in c if x.get("result")=="WIN"];l=[x for x in c if x.get("result")=="LOSS"];b=[x for x in c if x.get("result")=="BE"];r=sum(safe_float(x.get("result_r")) for x in c);p=sum(safe_float(x.get("result_r"))*safe_float(x.get("risk_amount")) for x in c);gp=sum(max(safe_float(x.get("result_r"))*safe_float(x.get("risk_amount")),0) for x in w);gl=abs(sum(min(safe_float(x.get("result_r"))*safe_float(x.get("risk_amount")),0) for x in l))
 ...
 def text(self):
  self.recalc();s=self.stats;return f"TRADES: {s['total']}\nWINS: {s['wins']} | LOSS: {s['losses']} | BE: {s['be']}\nWIN RATE: {s['win_rate']}%\nTOTAL R: {s['total_r']}\nPNL: {s['pnl']:.2f} USDT\nPF: {s['profit_factor']}\nACTIVE: {s['active']}"
TRACKER=PerformanceTracker();SCANNER=SignalScanner(BYBIT,TRACKER)
class PositionManager:
 def __init__(self,client,tracker,notify=None):self.client=client;self.tracker=tracker;self.notify=notify;self.running=False
 def start(self):
 ...
 def process(self,s):
  ...
  if (d=="long" and price<=sl) or (d=="short" and price>=sl):self.close(s,"SL",price);return
  if (d=="long" and price>=tp) or (d=="short" and price<=tp):self.close(s,"TP",price);return
  risk=safe_float(s["original_risk"]);entry=safe_float(s["entry"])
  if risk<=0:return
  r=(price-entry)/risk if d=="long" else (entry-price)/risk;s["current_r"]=round(r,4)
  if not s["partial_taken"] and r>=s["rr"]*Config.PARTIAL_TP_PERCENT/100:s["partial_taken"]=True;self.notify and self.notify.notify_partial(s)
  if not s["breakeven"] and r>=Config.BREAKEVEN_AFTER_R:s["sl"]=entry;s["breakeven"]=True;self.notify and self.notify.notify_breakeven(s)
 ...
 def trailing(self,s,price):
  ...
  if s["breakeven"]:new=max(new,s["entry"]) if d=="long" else min(new,s["entry"])
 ...
 def close(self,s,reason,price):
  ...
  if result=="BE":r=0
  if self.tracker.close(s["id"],result,r,price,reason,cost) and self.notify:self.notify.notify_close(s,result,reason,price,r)
...
class TelegramManager:
 ...
 def send(self,text,chat_id=None):
  ...
  try:
   r=requests.post(f"https://api.telegram.org/bot{self.token}/sendMessage",json={"chat_id":str(chat_id or self.chat),"text":text},timeout=Config.REQUEST_TIMEOUT);return r.status_code==200 and r.json().get("ok",False)
  except:return False
 def signal(self,s):return self.send(f"🚨 SWING AI {s['direction'].upper()}\n\n{s['symbol']}\nEntry: {s['entry']:.8g}\nSL: {s['sl']:.8g}\nTP: {s['tp']:.8g}\nRR: 1:{s['rr']:.2f}\nScore: {s['score']}/100\nLeverage: {s['leverage']}x\nRisk: {s['risk_amount']:.2f} USDT\n\nBOS → RETEST → CONFIRMATION")
...
class ScannerWorker:
 ...
 def symbols(self):
  try:
   ...
   a.sort(key=lambda x:x[1],reverse=True);return [x[0] for x in a[:Config.SCAN_TOP_N]] or Config.FALLBACK_COINS
  except:return Config.FALLBACK_COINS
 def loop(self):
  while self.running and not STOP_EVENT.is_set():
   try:
    syms=self.symbols();results=SCANNER.scan(syms);sent=0
    for s in results:
     if sent>=Config.MAX_SIGNALS_TO_SEND:break
     if TRACKER.add(s):TELEGRAM.signal(s);sent+=1
...
def main():
 validate_config();log.info("="*45);log.info("SWING AI BOT %s",BOT_VERSION);log.info("4H + 1H + 15M | BALANCED | TOP 40 TURNOVER",);log.info("="*45)
 ...
 if app is not None:threading.Thread(target=flask_start,daemon=True,name="Flask").start()
