...
BOT_VERSION="10.8.2 STABLE DIAGNOSTIC FINAL"
...
class Config:
 ...
 MIN_ATR_PCT=.30;MAX_ATR_PCT=8.;MIN_VOLUME_RATIO=1.10;MIN_EMA_DISTANCE_PCT=.05
 ...
 CORRELATION_THRESHOLD=.85;MAX_CORRELATED_ACTIVE=2
 MAX_HOLD_HOURS=96;DATA_DIR="swing_bot_data";FLASK_PORT=int(os.getenv("PORT","10000"))
 REQUEST_TIMEOUT=15;CACHE_TTL=10;TICKER_CACHE_TTL=30;MAX_CLOSED_SIGNALS_KEPT=200
...
class Bybit:
 ...
 def get(self,path,params=None,key="",ttl=None):
  params=params or {};k=key or path+json.dumps(params,sort_keys=True);t=time.time()
  life=Config.CACHE_TTL if ttl is None else ttl
  with self.lock:
   z=self.cache.get(k)
   if z and t-z[0]<life:return z[1]
  try:
 ...
 def klines(self,symbol,tf,limit=300):
  j=self.get("/v5/market/kline",{"category":"linear","symbol":symbol,"interval":tf,"limit":limit},f"k:{symbol}:{tf}:{limit}")
 ...
 def ticker(self,symbol,fresh=False):
  k=f"tick:{symbol}"
  if fresh:
   with self.lock:self.cache.pop(k,None)
  j=self.get("/v5/market/tickers",{"category":"linear","symbol":symbol},k,0 if fresh else Config.TICKER_CACHE_TTL)
  try:return j["result"]["list"][0]
  except:return {}
 def instruments(self):
  j=self.get("/v5/market/instruments-info",{"category":"linear","limit":1000},"inst",3600)
  return j.get("result",{}).get("list",[]) if j else []
 def funding(self,symbol):
  j=self.get("/v5/market/funding/history",{"category":"linear","symbol":symbol,"limit":1},f"fund:{symbol}")
  try:return safe(j["result"]["list"][0]["fundingRate"],math.nan)
 ...
 def spread(self,symbol):
  x=self.ticker(symbol);b=safe(x.get("bid1Price"),math.nan);a=safe(x.get("ask1Price"),math.nan)
  return abs(a-b)/b*100 if math.isfinite(a) and math.isfinite(b) and b>0 else math.nan
BYBIT=Bybit()
def top_symbols():
 deny=("AAPL","TSLA","NVDA","AMZN","GOOG","GOOGL","META","MSFT","MSTR","COIN","NFLX","AMD","INTC","PLTR","HOOD","ORCL","AVGO","QCOM","IBM","BA","DIS","NKE","PFE","XOM","CVX","GLD","SLV","SILVER","GOLD","OIL","WTI","BRN","COPPER")
 bad=("1000","10000","100000","1000000","10000000")
 vals=[]
 for x in BYBIT.instruments():
  s=x.get("symbol","")
  if x.get("status")!="Trading" or x.get("quoteCoin")!="USDT" or x.get("settleCoin")!="USDT" or x.get("contractType")!="LinearPerpetual":continue
  if any(s.startswith(z) for z in bad) or any(z in s for z in deny):continue
  v=safe(BYBIT.ticker(s).get("turnover24h"))
...
class Indicators:
 ...
 def add(df):
  ...
  delta=d.close.diff();gain=delta.clip(lower=0).rolling(Config.RSI_PERIOD).mean();loss=(-delta.clip(upper=0)).rolling(Config.RSI_PERIOD).mean()
  rs=gain/loss.replace(0,np.nan);d["rsi"]=(100-100/(1+rs)).replace([np.inf,-np.inf],np.nan).fillna(50)
  tr=pd.concat([d.high-d.low,(d.high-d.close.shift()).abs(),(d.low-d.close.shift()).abs()],axis=1).max(axis=1)
  d["atr"]=tr.rolling(Config.ATR_PERIOD).mean();d["atr_pct"]=d.atr/d.close*100
  d["volume_ma"]=d.volume.rolling(Config.VOLUME_LOOKBACK).mean();d["volume_ratio"]=d.volume/d.volume_ma.replace(0,np.nan)
  d["ema_distance_pct"]=(d.ema_fast-d.ema_slow)/d.close*100
  return d.replace([np.inf,-np.inf],np.nan).dropna().reset_index(drop=True)
...
class Structure:
 ...
 def swings(df,left=2,right=2):
  ...
  for i in range(left,len(df)-right):
   if df.high.iloc[i]>=df.high.iloc[i-left:i+right+1].max():sh.append((i,safe(df.high.iloc[i])))
   if df.low.iloc[i]<=df.low.iloc[i-left:i+right+1].min():sl.append((i,safe(df.low.iloc[i])))
  return sh,sl
 @staticmethod
 def trend(df):
  sh,sl=Structure.swings(df)
  if len(sh)<2 or len(sl)<2:return "neutral"
  hh=sh[-1][1]>sh[-2][1];hl=sl[-1][1]>sl[-2][1];lh=sh[-1][1]<sh[-2][1];ll=sl[-1][1]<sl[-2][1]
...
class Regime:
 ...
 def analyze(df):
  ...
  x=df.iloc[-1];st=Structure.trend(df);dist=abs(safe(x.ema_distance_pct));atr=safe(x.atr_pct)
  el=x.close>x.ema_fast and x.ema_fast>x.ema_slow and dist>=Config.MIN_EMA_DISTANCE_PCT
  es=x.close<x.ema_fast and x.ema_fast<x.ema_slow and dist>=Config.MIN_EMA_DISTANCE_PCT
  lo=el and st!="bearish";so=es and st!="bullish"
  bonus=45 if st in ("bullish","bearish") else 25
  q=min(100,min(dist,1)*35+min(atr/3,1)*20+bonus)
  if lo:return {"direction":"long","quality":q,"structure":st}
  if so:return {"direction":"short","quality":q,"structure":st}
...
class Strategy:
 ...
 def strong(df,i,d):
  if i<1:return False
  x=df.iloc[i];p=df.iloc[i-1];atr=safe(x.atr);body=abs(x.close-x.open)
  if atr<=0:return False
  return (x.close>x.open and body>=atr*.35 and x.close>=p.close) if d=="long" else (x.close<x.open and body>=atr*.35 and x.close<=p.close)
 @staticmethod
 def bos(df,d):
  ...
  for si,level in sw:
   ...
   for i in range(si+1,n):
    ...
    cb=x.close>=level+buf if d=="long" else x.close<=level-buf
    crossed=x.high>=level if d=="long" else x.low<=level
    rb=False
    if Config.ALLOW_RECLAIM_BOS and crossed:rb=Strategy.strong(df,i,d) and (x.close>=level if d=="long" else x.close<=level)
    if cb or rb:
     z={"level":level,"bar":i,"swing":si,"type":"CLOSE_BOS" if cb else "RECLAIM_BOS"}
     if best is None or i>best["bar"]:best=z
     break
 ...
 def retest_confirm(df,bos,d):
  ...
  atr=safe(df.atr.iloc[-1]);n=len(df)
  if atr<=0:return None
  ret=None
  for i in range(bos["bar"]+1,min(n-1,bos["bar"]+Config.RETEST_MAX_BARS)+1):
   ...
   if touch:ret=i;break
  if ret is None:return None
  for i in range(ret,min(n-1,ret+Config.CONFIRMATION_MAX_BARS_AFTER_RETEST)+1):
   if Strategy.strong(df,i,d):return {"retest_bar":ret,"confirm_bar":i}
  return None
 @staticmethod
 def entry_ok(df,d):
  atr=safe(df.atr.iloc[-1]);e=safe(df.close.iloc[-1]);ema=safe(df.ema_fast.iloc[-1])
  if atr<=0:return False
  return e<=ema+atr*Config.MAX_ENTRY_EXTENSION_ATR if d=="long" else e>=ema-atr*Config.MAX_ENTRY_EXTENSION_ATR
...
class Filters:
 ...
 @staticmethod
 def volume(df):return safe(df.volume_ratio.iloc[-1],0)>=Config.MIN_VOLUME_RATIO
 @staticmethod
 def funding(symbol):
  x=BYBIT.funding(symbol);return math.isfinite(x) and abs(x)<=Config.MAX_ABS_FUNDING
 @staticmethod
 def oi(symbol):
...
class Filters:
 ...
 def btc(symbol,d):
  ...
  x=Indicators.add(df).iloc[-1]
  return bool(x.close>x.ema_fast>x.ema_slow) if d=="long" else bool(x.close<x.ema_fast<x.ema_slow)

class Correlation:
 @staticmethod
 def allowed(symbol,active):
  if not Config.CORRELATION_FILTER_ENABLED or not active:return True
  try:
   a=BYBIT.klines(symbol,"60",100)
   if len(a)<50:return False
   ar=a.close.pct_change().dropna()
   for s in active:
...
class Store:
 def __init__(self):
  self.lock=threading.RLock();self.active=loadj(Config.SIGNAL_FILE,[]);self.closed=loadj(os.path.join(Config.DATA_DIR,"closed.json"),[])
  self.rejections={};self.errors={};self.stages={};self.bos_debug={};self.day=now().date().isoformat();self.daily_count=0
 def reset_day(self):
  d=now().date().isoformat()
  with self.lock:
   if d!=self.day:self.day=d;self.daily_count=0
 def reset_rejections(self):
  with self.lock:self.rejections={};self.errors={};self.stages={};self.bos_debug={}
 def add_stage(self,n,ok):
  with self.lock:
   z=self.stages.setdefault(n,{"pass":0,"fail":0});z["pass" if ok else "fail"]+=1
 def reject(self,s,r):
  with self.lock:self.rejections[s]=r or "UNKNOWN"
 def error(self,s,e):
  with self.lock:self.errors[s]=str(e)
 def rejection_stats(self):
  with self.lock:
   z={}
   for r in self.rejections.values():z[r]=z.get(r,0)+1
   return z
 def error_report(self):
  with self.lock:
   if not self.errors:return "Yoxdur"
   return "\n".join(f"{s}: {e[:160]}" for s,e in list(self.errors.items())[:10])
 def add_bos_debug(self,n):
  with self.lock:self.bos_debug[n]=self.bos_debug.get(n,0)+1
 def bos_report(self):
  with self.lock:return "Yoxdur" if not self.bos_debug else " | ".join(f"{k}: {v}" for k,v in sorted(self.bos_debug.items(),key=lambda z:-z[1]))
 def stage_report(self,n=0):
  ...
  return "\n".join(f"{x}: {q.get(x,{}).get('pass',0)}/{sum(q.get(x,{}).values())} keçdi | {q.get(x,{}).get('fail',0)} keçmədi" for x in order)
 def can_add(self):
 ...
 def persist(self):
  savej(Config.SIGNAL_FILE,self.active);savej(os.path.join(Config.DATA_DIR,"closed.json"),self.closed)
 def add(self,x):
  with self.lock:
   if not self.can_add() or any(a["symbol"]==x["symbol"] for a in self.active):return False
   x["status"]="ACTIVE";x["created"]=now().isoformat();self.active.append(x);self.daily_count+=1;self.persist();return True
STORE=Store()
...
def analyze_symbol(symbol):
 try:
  ...
  reg=Regime.analyze(d4);d=reg["direction"];ok=d!="neutral";STORE.add_stage("4H_TREND",ok)
  ...
  age=len(d15)-1-rc["confirm_bar"];ok=age<=Config.MAX_CONFIRM_AGE;STORE.add_stage("CONFIRM_AGE",ok)
  ...
  score=Score.calc(d4,d1,d15,reg,bos,rc);ok=score>=Config.MIN_SCORE;STORE.add_stage("SCORE",ok)
  if not ok:return None,"SCORE"
  x=d15.iloc[-1]
  return {"symbol":symbol,"direction":d.upper(),"entry":lv["entry"],"sl":lv["sl"],"tp":lv["tp"],"risk":lv["risk"],"rr":lv["rr"],"score":round(score,1),"rsi":safe(x.rsi),"atr_pct":safe(x.atr_pct),"volume_ratio":safe(x.volume_ratio),"bos_type":bos["type"],"bos_level":bos["level"],"confirm_age":age,"trend_structure":reg["structure"],"trend_quality":round(reg["quality"],1)},None
 except Exception as e:
  STORE.error(symbol,e);log.exception("analyze %s",symbol);return None,"ERROR"
...
class Telegram:
 ...
 def send(text):
  ...
  try:
   u=f"https://api.telegram.org/bot{Config.BOT_TOKEN}/sendMessage";r=requests.post(u,data={"chat_id":Config.CHAT_ID,"text":text},timeout=10)
   if not r.ok:log.error("TELEGRAM %s %s",r.status_code,r.text[:200])
   return r.ok
  except Exception as e:log.error("TELEGRAM %s",e);return False
 @staticmethod
 def signal(x):
  side="🟢 LONG" if x["direction"]=="LONG" else "🔴 SHORT"
  return Telegram.send(f"🚨 SWING SIGNAL {side}\n\nCoin: {x['symbol']}\nEntry: {x['entry']:.8g}\nSL: {x['sl']:.8g}\nTP: {x['tp']:.8g}\nRR: 1:{x['rr']:.2f}\nScore: {x['score']:.1f}\nRSI: {x['rsi']:.1f}\nATR: {x['atr_pct']:.2f}%\nVolume: {x['volume_ratio']:.2f}x\nBOS: {x['bos_type']}\n4H: {x['trend_structure']}")
 @staticmethod
 def scan_done(n,found,sent):
  q=STORE.rejection_stats();r=" | ".join(f"{k}: {v}" for k,v in sorted(q.items(),key=lambda z:-z[1])) if q else "Yoxdur"
...
class PositionManager:
 ...
 def check(self):
  ...
  for x in items:
   t=BYBIT.ticker(x["symbol"],True);p=safe(t.get("lastPrice"))
   if p<=0:continue
   hit=None
   if x["direction"]=="LONG":
    if p<=x["sl"]:hit="SL"
   ...
   expired=False
   try:expired=(now()-datetime.fromisoformat(x["created"])).total_seconds()>Config.MAX_HOLD_HOURS*3600
   except:pass
 ...
 def close(self,x,result,p):
  with STORE.lock:
   ...
   z=dict(x);z.update(status="CLOSED",result=result,exit=p,closed=now().isoformat())
   STORE.closed.append(z);STORE.closed=STORE.closed[-Config.MAX_CLOSED_SIGNALS_KEPT:];STORE.persist()
  em="✅" if result=="TP" else "❌" if result=="SL" else "⏱"
  Telegram.send(f"{em} SIGNAL {result}\n\n{x['symbol']} {x['direction']}\nEntry: {x['entry']:.8g}\nExit: {p:.8g}\nSL: {x['sl']:.8g}\nTP: {x['tp']:.8g}")
  ...
  try:
   ...
   with ThreadPoolExecutor(max_workers=Config.PARALLEL_WORKERS) as ex:
    ...
    for f in as_completed(fs):
     s=fs[f]
     try:x,r=f.result()
     except Exception as e:x,r=None,"ERROR";STORE.error(s,e)
     if x:found.append(x)
     else:STORE.reject(s,r)
   ...
   for x in found:
    ...
    if not Correlation.allowed(x["symbol"],STORE.active_symbols()):
     STORE.reject(x["symbol"],"CORRELATION");continue
    if STORE.add(x):
...
@app.get("/status")
def status():
 return jsonify({"version":BOT_VERSION,"running":SCANNER.running,"active":STORE.active,"closed":len(STORE.closed),"daily":STORE.daily_count})

@app.get("/signals")
def signals():return jsonify(STORE.active)
...
@app.get("/rejections")
def rejections():
 return jsonify({"stats":STORE.rejection_stats(),"details":STORE.rejections,"errors":STORE.errors,"stages":STORE.stages,"bos":STORE.bos_debug})

def start():
 Telegram.send(f"🤖 {BOT_VERSION}\nBot aktivdir.\n4H → 1H → 15M\nMIN SCORE: {Config.MIN_SCORE}\nRR: {Config.MIN_RR}-{Config.MAX_RR}")
